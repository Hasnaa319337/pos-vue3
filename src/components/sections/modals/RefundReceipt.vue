<template>
  <section>
    <b-modal
      id="refund-receipt-modal"
      hide-footer
      class="refund-receipt-modal"
      ref="refund-receipt-modal"
      centered
      scrollable
      size="xl"
    ...
    >
      ...
      >
        <div v-if="invoiceID > 0">
          ...
          >
            ...
            <div class="col-auto p-0">
              <h2 class="left_header">
                <!-- {{ refund.provider.name ? refund.provider.name : '' }} -->
                {{ provider.name }}
              </h2>
              <p class="paragraph_left_header">
                <span>{{ trans('customer address') }} : </span>
                <span>
                  {{ _.get(refund, 'provider.info.billing_address.city_name') }}
                  -
                  {{ _.get(refund, 'provider.info.billing_address.region_name') }}
                  -
                  {{ _.get(refund, 'provider.info.billing_address.street_name') }}
                </span>
              </p>
              <p class="paragraph_left_header">
          ...
          <div class="row justify-content-end m-0 products">
            <div class="col-12 header table_header_div">
              <div class="row align-items-center m-0">
                <div class="col table_header_div_chlid" style="padding: 0 5px; width: 15%">
                  {{ trans('barcode') }}
                </div>
                <div class="col table_header_div_chlid" style="padding: 0 5px; width: 15%">
                  {{ trans('product name') }}
                </div>
                <div class="col table_header_div_chlid" style="padding: 0 5px; max-width: 90px">
                  {{ trans('unit') }}
                </div>
                <div class="col table_header_div_chlid" style="padding: 0 5px; max-width: 90px">
                  {{ trans('quantity') }}
                ...
                <div class="col table_header_div_chlid" style="max-width: 90px; padding: 0 5px">
            ...
            <div class="col-12 p-0">
              <div
                :style="index % 2 !== 0 ? 'background: #F2F2F2' : 'background: transparent'"
                v-for="(item, index) in items"
                :key="index"
                class="row align-items-center m-0 table_div_body"
              >
                <div class="col" style="width: 15%; padding: 0 5px">
                  {{ item.serial }}
                </div>
                <div class="col" style="padding: 0 5px; width: 15%">
                  {{ item.title }}
                </div>
                ...
                <div class="col" style="padding: 0 5px">% {{ item.discount_percentage }}</div>
                <div class="col" style="padding: 0 5px">
                  {{
                ...
                <div class="col" style="padding: 0 5px">% {{ item.tax_percentage }}</div>
                <div class="col" style="padding: 0 5px; max-width: 90px">
                  {{
          ...
          >
            ...
            <div class="col-sm-auto col-12 totals mb-3" style="max-width: 50%">
              ...
              >
                <b
                  ...
                  style="
                    ...
                    color: #137b97;
                  "
                  >{{ trans('adding_added_tax') }} ({{ refund.items[0].tax_percentage }}
                  %)
                </b>
                <b class="px-3" style="letter-spacing: 0px; color: #394e85">
              ...
              >
                ...
                <b class="px-3" style="letter-spacing: 0px; color: #394e85" v-else>
                  {{ trans('installment') }}
                </b>
              </div>
            </div>
          ...
          >
            <div class="col-auto">
              <p style="color: rgb(19, 123, 151); font-size: 21px !important; font-weight: 700">
        ...
        <div v-else>
          <i class="fa fa-spinner fa-spin" style="font-size: 80px; margin-top: 50px"></i>
        </div>
      </div>

      <!-- start:: printed invoice -->
      <global-refund :refund="refund" :settings="settings" id="globalRefundInvoice" />
      <!-- End:: printed invoice -->
...
<script>
import GlobalRefund from '../../global/invoice/GlobalRefund.vue'

export default {
  components: { GlobalRefund },
  props: ['settings', 'user', 'invoice'],
  data() {
    return {
      customEvents: [
        {
          eventName: 'open-refund-receipt-modal',
          callback: this.openRefundReceiptModal
        }
      ],
      message: '',
      alertType: '',
...
* {
  ...
  methods: {
    openRefundReceiptModal(data) {
      ...
      this.$refs['refund-receipt-modal'].show()
    },
    closeRefundReceiptModal() {
      this.$bvModal.hide('refund-receipt-modal')

      this.$router.push('/refund-purchases')
    },
    printReceipt() {
      //invoke Print me From Mixin //
      this.printMe('#globalRefundInvoice', this.cssText)
      this.$bvModal.hide('refund-receipt-modal')
    }
  }
}
