<template>
  <section style="overflow-y: scroll" class="container-fluid" v-if="loaded">
    <template v-if="custom_code === 4000">
      <div>
        <div
          style="
      ...
      <b-row>
          ...
          <div id="invoice_title" class="col-sm-12">
            <b-row>
              <b-col>
                <h3 class="mb-3">
                  <!-- purchase -->
                  {{ trans('refund purchase invoice') }} # {{ invoice.id }}
                </h3>
                <h5 class="mb-3">
                  {{
        ...
        <b-col class="12">
          <div class="row">
            ...
            <!-- 
            <div class="col" v-if="invoice.payment_type === 'cash'">
      ...
      >
        ...
        <b-tbody>
          ...
          >
            ...
            <b-td>{{ item.cost - item.discount_amount }}</b-td>
            ...
            <td>
              <b-form-row class="justify-content-center" style="gap: 5px">
                <b-input
                  type="text"
...
import RefundReceipt from './modals/RefundReceipt.vue'
...
export default {
  ...
  computed: {
    ...
    itemsSubtotal() {
      if (this.refund.length) {
        ...
        for (let i = 0; i < this.refund.length; i++) {
          this.refund[i].subtotal =
            parseFloat(this.refund[i].cost - this.refund[i].discount_amount) *
            parseInt(this.refund[i].quantity)
  ...
  mounted() {
    this.getInvoiceData(localStorage.getItem('myRefundPurchaseInv'))
    this.refresh()
    this.getProducts()
  },
  methods: {
        ...
        itemsData.map(item => {
          ...
          item.grouped.map(subItem => {
            ...
            subItem['valueAfterDiscount'] =
              ...
              parseFloat(subItem.cost - subItem.discount_amount).toFixed(2)
    ...
    getInvoiceData(id) {
      this.loaded = true
      axios
        .get('/api/purchase-refund/load-invoice/' + id)
        .then(res => {
          this.invoice = res.data.body.invoice
          this.AllAccounts(this.invoice.payment_method)
          localStorage.removeItem('myRefundPurchaseInv')
        })
        .catch(error => {
          console.log(error)
          this.custom_code = error.response.data.custom_code
          console.log(this.custom_code)
        })
    },
    clearForm() {
      this.refund = []
    ...
    refundAll(index, quantity) {
      ...
      console.log('refund all')
      console.log(this.refund)
      console.log('refund length')
      console.log(this.refund.length)
    },
    ...
    submitRefund() {
      ...
      if (this.invoice.treasury_account && this.refund.length) {
        axios
          .post('/api/purchase-refunds', data)
          .then(res => {
            ...
            this.$router.push(
              '/refund-purchase/view/' + res.data.body.purchase_refund.id
            )
          })
          .catch(error => {
            this.showToastError(
