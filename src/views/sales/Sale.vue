<template>
  <div class="col-sm-12 grid-margin stretch-card">
    <div class="card pt-3" v-if="loaded && permissions.includes('view_sale')">
      <div
            ...
            <b-row class="mb-3 mx-0">
              ...
              <b-col cols="12" md="4">
                ...
                >
                  ...
                  <span>{{ trans(sale.payment_method) }}</span>
                </div>
              </b-col>
            </b-row>
            ...
            <b-row class="mx-0">
              ...
              >
                ...
                <b-tbody v-for="(item, index) in sale.items" :key="item.id">
                  <b-tr>
                    ...
                    <b-td>
                      ...
                      <!-- {{ ((item.price - item.discount_amount) * item.quantity) + (((item.price - item.discount_amount) * item.quantity) * item.tax_percentage / 100) }} -->
                    </b-td>
                  </b-tr>
                </b-tbody>
                <b-tr
                ...
                <b-tbody>
                  ...
                  <!-- addedTax * total taxes* -->
                  <b-tr v-for="item in sale.items" :key="item">
...
export default {
  ...
  components: { SaleReceipt, Refund, GoogleMap, SkeletonLoader },
  data() {
    return {
      columns: [
        'id',
  ...
  methods: {
    refresh() {
      let url = '/api/sales/' + this.$route.params.id
      this.loaded = false
      axios
        .get(url)
        .then(res => {
