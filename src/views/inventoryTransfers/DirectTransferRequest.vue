<template>
  <div class="col-sm-12 grid-margin stretch-card">
    <div class="card" v-if="items.length > 0">
      ...
      >
        <h4 class="card-title">
          {{ trans('Store transfer voucher') }} ( {{ id }} #)
        </h4>
        <GoBackButton />
      </div>
      <div style="margin: 11px 7px 0">
        ...
        <h4 style="display: inline-block; margin: 0 8px">
          <span style="font-weight: 600" :style="{ color: color }"
            >{{ trans('voucher') }} {{ trans(globalStatus) }}
          </span>
        </h4>
      </div>
      ...
      <div class="d-flex" style="height: 70px">
        <div class="filter col-sm-6" style="margin-bottom: 35px">
          <b-form-input
            ...
            readonly
            type="text"
            v-model="inventory_transfer.from_inventory.title"
          >
          </b-form-input>
          <span
            style="
        ...
        <div class="filter col-sm-6" style="margin-bottom: 35px">
          <b-form-input
            ...
            readonly
            type="text"
            v-model="inventory_transfer.note"
          >
          </b-form-input>
          <span
            style="
      ...
      <div class="d-flex" style="height: 70px">
        <div class="filter col-sm-6" style="margin-bottom: 35px">
          <b-form-input
            ...
            readonly
            type="text"
            v-model="inventory_transfer.to_inventory.title"
          >
          </b-form-input>
          <span
            style="
        ...
        <div class="filter col-sm-6" style="margin-bottom: 35px">
          <b-form-input
            ...
            readonly
            type="text"
            v-model="inventory_transfer.created_at"
          >
          </b-form-input>
          <span
            style="
              ...
              >
                ...
                <b-tbody v-if="items.length > 0">
                  <b-tr v-for="(request, index) in items" :key="request.id">
                    <b-td>{{ index + 1 }}</b-td>
                    <b-td>
                      {{ spliceText(request.product.title, '10') }}
                    </b-td>
                    <b-td style="width: 130px !important">
                      {{ spliceText(request.product.serial, '10') }}
                    </b-td>
...
export default {
  ...
  methods: {
    ...
    getMyTransferRequests() {
      axios
        .get(
          '/api/inventory-transfer/inventory-transfers/' + this.$route.params.id
        )
        .then(res => {
          this.loaded = true
          this.items = res.data.body.inventory_transfer.items
          this.globalStatus = res.data.body.inventory_transfer.status.value
          this.inventory_transfer = res.data.body.inventory_transfer
          this.color = res.data.body.inventory_transfer.status.color.hexadecimal
          this.note = res.data.body.inventory_transfer.note
          this.from_inventory = res.data.body.inventory_transfer.from_inventory
          this.to_inventory = res.data.body.inventory_transfer.to_inventory
          this.created_at = res.data.body.inventory_transfer.created_at
          this.created_by = res.data.body.inventory_transfer.created_by
        })
        .catch(error => {
          this.showToastError(
